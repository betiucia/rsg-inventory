...
local function CreateItemDrop(index)
    ...
    FreezeEntityPosition(dropItem, false)
...
RegisterNUICallback("ClipItem", function()
    ...
    lib.registerContext({
        ...
        options = {
          ...
          {
            title = 'Dinheiro Legal',
            description = 'Faça um maço de dinheiro legal',
            event = 'inventory:client:MoneyClip',
            arrow = true,
            args = {}
          },
          {
            title = 'Dinheiro Sujo',
            description = 'Faça um maço de dinheiro sujo',
            event = 'inventory:client:BloodMoneyClip',
            arrow = true,
            args = {}
          }
        }
      ...
      lib.showContext('money_clip_menu')

end)

RegisterCommand('jomenu', function()
    print('toabrino')
    TiggerEvent('snippet_menu:client:carregarmenu')
end)


RegisterNetEvent('inventory:client:MoneyClip', function()
    local input = lib.inputDialog('Deseja fazer um maço de dinheiro?', {
        {type = 'input', label = 'Quatidade em $', description = 'digite aqui o valor', icon = 'fa-solid fa-money-bills'},
      })
    
    if not input then return end
    input = input[1]
    TriggerServerEvent('inventory:server:moneyclip',input)
end)

RegisterNetEvent('inventory:client:BloodMoneyClip', function()
    local input = lib.inputDialog('Deseja fazer um maço de dinheiro sujo?', {
        {type = 'input', label = 'Quatidade em $', description = 'digite aqui o valor', icon = 'fa-solid fa-money-bills'},
      })
    
    if not input then return end
    input = input[1]
    TriggerServerEvent('inventory:server:bloodmoneyclip',input)
end)
