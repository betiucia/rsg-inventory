...
RegisterNetEvent('inventory:server:moneyclip', function(input)
-- RSGCore.Commands.Add('moneyclip2', Lang:t('server.lang_6'), {{ name = 'amount', help = Lang:t('server.lang_7') }}, true, function(source, args)
    local src = source
    args = tonumber(input)
    local args1 = args

    if args1 == nil then
        TriggerClientEvent('ox_lib:notify', source, {title = 'Isso não é um valor correto', type = 'error', duration = 7000 })
        return
    end

    if args1 <= 0 then
        TriggerClientEvent('ox_lib:notify', source, {title = 'Você não possui dinheiro', type = 'error', duration = 7000 })
        return
    end
    ...
    if money and money >= args1 then
        if Player.Functions.RemoveMoney('cash', args1, 'give-money') then
            ...
            TriggerClientEvent('ox_lib:notify', source, {title = 'Deu Tudo certo',type = 'success', duration = 7000 })
        else
            TriggerClientEvent('ox_lib:notify', source, {title = 'Você não possui esse dinheiro', type = 'error', duration = 7000 })
        end
    end
end, 'user')

RegisterNetEvent('inventory:server:bloodmoneyclip', function(input)
    -- RSGCore.Commands.Add('moneyclip2', Lang:t('server.lang_6'), {{ name = 'amount', help = Lang:t('server.lang_7') }}, true, function(source, args)
        local src = source
        args = tonumber(input)
        local args1 = args
